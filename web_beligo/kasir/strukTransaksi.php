				            ...
				            <table class="table table-striped table-bordered" width="80%">
								...
								<tr>
					                <td colspan="2">
					                	...
					                	<?include "kd_transaksi"?>
                                            <?php  
                                            //<-- LOKASI FILE UTAMA PLUGINNYA
                                            $tempdir = "temp/"; 
                                            //<-- Nama Folder file QR Code kita nantinya akandisimpan
                                            if (!file_exists($tempdir))
                                            #kalau folder belum ada, maka buat.    
                                                mkdir($tempdir);
                                            $isi_teks = $dd['kd_transaksi'].$dd['tanggal_beli'];
                                            $filename = $isi_teks.".png";
                                            $quality = 'H'; 
                                            //ada 4 pilihan, L (Low), M(Medium), Q(Good), H(High)
                                            $ukuran = 5; 
                                            //batasan 1 paling kecil, 10 paling besar
                                            $padding = 0;
                                            QRCode::png($isi_teks,$tempdir.$filename,$quality,$ukuran,$padding);
                                            ?>
                                            <img width="100px" heigth="100px" src="temp/<?php echo $filename; ?>"></td>
					                </td>
